...
namespace GZ.Projects.WCS_YL
{
	...
	/// 任务确认反馈辅助类
	/// </summary>
	public class StackerConfirm
	{
		
        /// <summary>
        /// 默认构造函数。
        /// </summary>
		public StackerConfirm()
		{
		}
		
		/// <summary>
		/// 统一处理堆垛机状态反馈：读取信号、调用对应处理方法并输出日志。
		/// </summary>
		/// <param name="num">堆垛机编号</param>
		public void HandleStackerStatusFeedback(string num)
		{
			DateTime startTime = DateTime.Now;
			string msg = $"\r\n\t[{num}号堆垛机任务信号反馈：Start{startTime:yyyy/MM/dd HH:mm:ss:fff}]";
			...
			try
			{
				...
				msg += $"获取wcs的确认信号[{wcsConfirmSignal}]，plc状态当前值为[{plcTaskStatus}]]{DateTime.Now:yyyy/MM/dd HH:mm:ss}";

				if (wcsConfirmSignal != "0" && plcTaskStatus == "0")
				{
					helperResult = InteractionFeedback(num);
					isLog = true;
				}
				else if (wcsConfirmSignal == "0" && plcTaskStatus != "0")
				{
					...
					{
						case "1":
							helperResult = NormalFinishFeedback(num);
							break;
						case "2":
							helperResult = ForceFinishFeedback(num);
							break;
						case "3":
							helperResult = CancelFeedback(num);
							break;
						default:
							msg += $"\r\n\t[{suffixName}堆垛机任务完成反馈信号未满足要求]End";
							helperResult = new TaskConfirmResult(false, string.Empty);
							break;
					}
				}
				else
			...
			finally
			{
				...
				if (isLog)
				{
					if (helperResult != null && helperResult.IsSuccess)
					{
